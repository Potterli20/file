...
# When using "su" an incomplete PATH could be passed: https://hub.fastgit.xyz/pi-hole/pi-hole/issues/3209
...
webInterfaceGitUrl="https://hub.fastgit.xyz/pi-hole/AdminLTE.git"
webInterfaceDir="${webroot}/admin"
piholeGitUrl="https://hub.fastgit.xyz/pi-hole/pi-hole.git"
...
# See https://hub.fastgit.xyz/pi-hole/pi-hole/issues/1473#issuecomment-301745953
testIPv6() {
...
FTLinstall() {
    ...
    if [[ "${ftlBranch}" == "master" ]];then
        url="https://hub.fastgit.xyz/pi-hole/ftl/releases/latest/download"
    else
...
get_binary_name() {
    ...
    if [[ "${machine}" == "arm"* || "${machine}" == *"aarch"* ]]; then
        ...
        # in the past (see https://hub.fastgit.xyz/pi-hole/pi-hole/pull/2004)
...
FTLcheckUpdate() {
    ...
    if [[ ! "${ftlBranch}" == "master" ]]; then
        ...
        if [[ ${ftlLoc} ]]; then
            ...
            if ! FTLlatesttag=$(curl -sI https://hub.fastgit.xyz/pi-hole/FTL/releases/latest | grep --color=never -i Location: | awk -F / '{print $NF}' | tr -d '[:cntrl:]'); then
            ...
            if [[ "${FTLversion}" != "${FTLlatesttag}" ]]; then
                ...
                remoteSha1=$(curl -sSL --fail "https://hub.fastgit.xyz/pi-hole/FTL/releases/download/${FTLversion%$'\r'}/${binary}.sha1" | cut -d ' ' -f 1)
                localSha1=$(sha1sum "$(command -v pihole-FTL)" | cut -d ' ' -f 1)
    ...
    if [[ "${EUID}" -eq 0 ]]; then
        ...
        if is_command sudo ; then
            ...
            if [[ "$0" == "bash" ]]; then
                ...
                exec curl -sSL https://file-git.trli.club/pihole/install-pihole.sh | sudo bash "$@"
            else
