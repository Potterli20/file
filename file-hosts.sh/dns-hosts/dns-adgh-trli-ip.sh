...
function GenerateRules() {
    domestic_dns=(
        $(for protocol in tcp udp; do echo "${protocol}://192.168.3.145:53"; done)
        $(for protocol in tcp udp; do echo "${protocol}://192.168.3.147:53"; done)
        $(for protocol in tcp udp; do echo "${protocol}://192.168.3.148:53"; done)
    )
    foreign_dns=(
