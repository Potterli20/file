...
function OutputData() {
    function GetDNSRecord() {
        dns_record_alidns=$(curl -s --connect-timeout 15 -H "accept: application/dns-json" "https://dns.alidns.com/resolve?name=${dhdb_data[$dhdb_data_task]}&type=A)
        ...
        dns_record_google=$(curl -s --connect-timeout 15 -H "accept: application/dns-json" "https://dns.google/resolve?name=${dhdb_data[$dhdb_data_task]}&type=A)
    }
