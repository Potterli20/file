#!/usr/bin/env bash
# ad-hosts-lite.sh —— 聚合各广告/恶意域名过滤列表并生成多格式输出
#
# 用法: 在仓库根目录执行  bash ./file-hosts.sh/ad-hosts/ad-hosts-lite.sh
#       (workflow 与此一致; 产物 ./ad-* 输出到当前运行目录, 临时目录 ./ad-hosts-lite 用后即删)
#
# 主要优化:
...
function GetData() {
    ...
    mkdir -p ./ad-hosts-lite && cd ./ad-hosts-lite || exit 1
...
function GenerateInformation() {
    ...
    adfilter_title="trli's Ad Filter"
...
function OutputData() {
    ...
    if ((total_lines < MIN_DOMAINS)); then
        echo "[error] 最终域名仅 ${total_lines} 条, 疑似抓取大面积失败, 已中止以保护线上产物 (临时目录 ./ad-hosts-lite 保留供排查)" >&2
        exit 1
    fi
    ...
    if [[ -f ../ad-domains.txt ]]; then
        ...
        if cmp -s ./filter_data.tmp ./filter_data.old; then
            ...
            cd "$ORIG_PWD" && rm -rf ./ad-hosts-lite
            return 0
        fi
    fi
